...
#hel karp functin
def held_karp1(n_city,global_distance_matrix):
    n_cities = n_city
    distance_matrix  = global_distance_matrix[:]
    cost = {(1 << destination, destination): distance_matrix[0][destination]
            for destination in range(1, n_cities)}

    for subset_size in range(2,n_cities):
        for subset in itertools.combinations(range(1,n_cities),subset_size):
            subset_as_int = sum([1 << x for x in subset])
            for destination in subset:
                cost[subset_as_int,destination] = min([cost[subset_as_int & - (1 << destination), side_quest]
                                                        + distance_matrix[side_quest][destination]
                                                        for side_quest in subset
                                                        if side_quest != destination])
    return min([cost[(1 << n_cities) - 2, last_stop] + distance_matrix[last_stop][0]
                for last_stop in range(1,n_cities)])
...
n_days = int(input("Numero de dias que dispone: "))
n_places = int(input("Numero promedio de lugares que desea visitar: "))
